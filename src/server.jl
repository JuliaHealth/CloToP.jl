...
function handle(req)
    if req.method == "POST"
        ...
        return HTTP.Response(200, ["Access-Control-Allow-Origin"=>"http://0.0.0.0:8080", "Access-Control-Request-Method"=>"POST"], "$(clo_group) $(clo_group_adj) $(clo_level) $(nclo_level) $(dose_range[1]) $(dose_range[2]) $(p)")
    end
    return HTTP.Response(200, ["Access-Control-Allow-Origin"=>"http://0.0.0.0:8080", "Access-Control-Request-Method"=>"POST"], read("./index.html"))
end
