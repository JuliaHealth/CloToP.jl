...
if isfile("data/clozapine_test.csv")
    ...
    predict_raw_data = CSV.read("data/clozapine_test.csv", header=true, DataFrame)
else
...
prediction      ├──────┼──────┤
           high │ $(lpad(cm[3], 4, " ")) │ $(lpad(cm[1], 4, " ")) │
                └──────┴──────┘
         """)
println("\tsensitivity (TP): ", round(count(mode.(yhat2) .== "high") / count(y2 .== "high"), digits=2))
println("\tspecificity (TN): ", round(count(mode.(yhat2) .== "norm") / count(y2 .== "norm"), digits=2))
println()
...
prediction      ├──────┼──────┤
           high │ $(lpad(cm[3], 4, " ")) │ $(lpad(cm[1], 4, " ")) │
                └──────┴──────┘
         """)
println("\tsensitivity (TP): ", round(count(yhat2_adj .== "high") / count(y2 .== "high"), digits=2))
println("\tspecificity (TN): ", round(count(yhat2_adj .== "norm") / count(y2 .== "norm"), digits=2))
println()
...
p1 = Plots.plot(y1, label="CLO: data")
p1 = Plots.plot!(yhat1, label="CLO: prediction", line=:dot, lw=2)
p2 = Plots.plot(y3, label="NCLO: data")
p2 = Plots.plot!(yhat3, label="NCLO: prediction", line=:dot, lw=2)
